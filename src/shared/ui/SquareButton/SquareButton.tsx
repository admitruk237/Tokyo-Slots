...
export const SquareButton = ({ children, onClick, disabled = false, className = '' }: Props) => {
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'relative w-[63px] h-[59px] rounded-[16px] border-[2px] border-[#341D1A]',
        ...
        'active:shadow-none',
        'disabled:bg-[#969595] disabled:pointer-events-none',
        className
      )}
    >
      <div className="font-poetsen text-[25px] font-normal text-[#341D1A]">{children}</div>
    </button>
  );
};
